...
% 2024-01-17  RK  Added example 4 for creating a figure with multiple
%                 Bland-Altman plots
...
%% Example 4 - Two Bland-Altman plots in one figure
% This is a bit of a hack, as does not (yet) support BlandAltman-only mode,
% so draw on temporary figure and copy only the BA axes elements.
fig = figure('Color','w','Units','centimeters', 'Position',[2 2 20 10]);
ah1 = subplot(121);
ah2 = subplot(122);
tempFig = figure;

BAinfo = {'RPC(%)','ks','SSE','RMSE'};
baYLim = [-1 1]; % Set the Y axis (difference) limits on the Bland-Altman plots.

[~, tempFig] = BlandAltman(tempFig, data1, data2, label, tit, gnames,'corrInfo',corrinfo,'baInfo',BAinfo,'axesLimits',limits,'colors',colors,'baYlimMode',baYLim);
ah = copyobj(findobj(get(tempFig,'children'),'flat','tag','Bland Altman Plot'),fig);
ah.Position = ah1.Position;
delete(ah1)
title(ah,'Baseline')

% Repeat analysis with more variance on data2.
clf(tempFig);
[~, tempFig] = BlandAltman(tempFig, data1, data2.*(1+0.1*randn(size(data2))), label,[tit ' (with noise)'], gnames,'corrInfo',corrinfo,'baInfo',BAinfo,'axesLimits',limits,'colors',colors,'baYlimMode',baYLim);
ah = copyobj(findobj(get(tempFig,'children'),'flat','tag','Bland Altman Plot'),fig);
ah.Position = ah2.Position;
delete(ah2)
title(ah,'With noise')
close(tempFig)
