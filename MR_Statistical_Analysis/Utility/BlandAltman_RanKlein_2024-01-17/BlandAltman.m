...
if length(label)==1
	...
	params.deltaLabel = ['Difference ' label{1}];
...
mdl = fitlm(data.maskedSet1, data.maskedSet2); % , 'Intercept', false
SE_intercept = mdl.Coefficients{1,2};
stats.p_intercept = mdl.Coefficients{1,4};
SE_slope = mdl.Coefficients{2,2};
stats.p_slope = mdl.Coefficients{2,4};
tstat = tinv(0.975, stats.N-2);
stats.intercept_error = tstat * SE_intercept;
stats.slope_error = tstat * SE_slope;
stats.adjR2 = mdl.Rsquared.Adjusted;
	...
	switch upper(params.corrInfo{i})
		case 'EQ'
			...
			elseif stats.intercept>=0
				corrtext = [corrtext; ['y=' mynum2str(stats.slope,3,2) 'x+ ' mynum2str(stats.intercept,3,2)]];
			else
				corrtext = [corrtext; ['y=' mynum2str(stats.slope,3,2), 'x+ ' mynum2str(stats.intercept,3,2)]];
			end
		case 'SL'
			if ~strcmpi(params.forceZeroIntercept,'off')
				corrtext = [corrtext; ['y=(' mynum2str(stats.slope,3,2), '+/-' mynum2str(stats.slope_error,3,2), ')x']];
			elseif stats.intercept>=0
				corrtext = [corrtext; ['y=(' mynum2str(stats.slope,3,2), '+/-' mynum2str(stats.slope_error,3,2), ')x+', mynum2str(stats.intercept,3,2)]];
			else
				corrtext = [corrtext; ['y=(' mynum2str(stats.slope,3,2), '+/-' mynum2str(stats.slope_error,3,2), ')x', mynum2str(stats.intercept,3,2)]];
			end
		case 'ITCP'
			corrtext = [corrtext; ['a0=' mynum2str(stats.intercept,3,2), '+/-', mynum2str(stats.itcp_error,3,2)]];
		case 'R2'
			% if strcmp(params.corrInfo{i},'r2')
			corrtext = [corrtext; ['adj-R^2=' mynum2str(stats.adjR2,4)]];
			% else
			% corrtext = [corrtext; ['adj-R^2=' mynum2str(stats.adjR2,4)]];
			% end
		case 'R'
