...
for quad = i
    ...
    if matches(quad{1}, 'FER')
        ...
        data1             = DataFSR.FER(:,1).*1e3;
        data2             = DataFSR.FER(:,2).*1e3;
       % tit_3T = strcat(tit_fer, ', 3T');
        % [cr1, fig1, statsStruct1] = BlandAltman(data1, data2_3T, label, tit_3T, gnames, ...
    ...
    elseif matches(quad{1}, 'USP')
        ...
        % tit_usp = strcat(title_prefix, 'USPIO ROIs');
        % tit = strcat(param, 32, 'after', 32, title_prefix, ': USPIO ROIs') ; % 'Reproducibility, USPIO ROIs',
        % label = {'Truth', 'Measured', 'ppb'};
        % gnames = {label{1}, label{2}};
        
        if exist('hdr', 'var') || hdr.is3T
            data1             = DataFSR.USP(:,1) .* 1e3 .* 2.89; % ppb
            % tit_3T = strcat(tit_usp, ', 3T');
            % [cr3, fig3, statsStruct3] = BlandAltman(data1_3T, data2_3T, label, tit_3T, gnames, ...
            %     'corrInfo', corrinfo, 'baInfo', BAinfo, 'axesLimits', limits, ...
            %     'colors', colors, 'showFitCI', ' on', 'data1Mode', 'Truth');
        end
        if exist('hdr', 'var') || hdr.is7T
            data2_7T             = DataFSR.USP(:,2) .* 1e3 .* 7; % ppb
            % tit_7T = strcat(tit_usp, ', 7T');
