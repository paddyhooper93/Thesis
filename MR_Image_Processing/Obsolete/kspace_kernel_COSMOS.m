function [D, kappa] = kspace_kernel_COSMOS(varargin)
...
[ky, kx, kz] = meshgrid(-N(1)/2:N(1)/2-1, ...
    -N(2)/2:N(2)/2-1, ...
    -N(3)/2:N(3)/2-1);
...
%       
    ...
    if 1~=exist("B0_dir", "var")
        ...
        D(:,:,:,t) = 1/3 - ( kx*B0_dir(1,t) + ky*B0_dir(2,t) + kz*B0_dir(3,t) ).^2 ./ (k2 + eps);
    elseif 1~=exist("R_tot", "var")
        % R_tot: 3x3 rotation matrix, indexed to dim 3
        D(:,:,:,t) = 1/3 - ( kx*R_tot(3,1,t) + ky*R_tot(3,2,t) + kz*R_tot(3,3,t) ).^2 ./ (k2 + eps);
    end
    D(isnan(D)) = 0;
    D(:,:,:,t) = fftshift(D(:,:,:,t));
