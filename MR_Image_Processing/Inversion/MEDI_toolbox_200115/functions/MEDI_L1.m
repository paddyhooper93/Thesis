...
function [x, final_cost_data, final_cost_reg, D, resultsfile] = MEDI_L1(varargin)
...
b0 = m.*exp(1i*RDF);
[wG] = gradient_mask(iMag, Mask, grad, voxel_size, percentage, gradient_weighting_mode);
...
if (smv)
    fprintf('SMV kernel used\n');
end
oldN_std=N_std;
fprintf(['Using ' solver '\n']);
switch solver
    case 'gaussnewton'
        [x, final_cost_reg, final_cost_data] = gaussnewton();
end

    function [x, final_cost_reg, final_cost_data] = gaussnewton()
        ...
        final_cost_data = cost_data_history(iter);
        final_cost_reg = cost_reg_history(iter);
