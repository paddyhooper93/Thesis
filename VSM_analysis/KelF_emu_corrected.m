%% KelF_emu_corrected.m
%% Script does the following:
% Calculates the susceptibility (slope), remanence (y-intercept) and coercivity (x-intercept)
% Subtracts the susceptibility (slope) of the diamagnetic sample holder
% Correct for the demagnetizing field (longitudinal cylinder parallel to B0)
% N_d = (1 + 1.6*length/diameter)^(-1);
% m_cor(x) = m(x) / (1+m(x)*N_d);
...
N_B = (1 + 1.6*length_B/(2*radius_B))^(-1);
N_A = (1 + 1.6*length_A/(2*radius_A))^(-1);
N_d = mean([N_A,N_B]);
...
for y = i
    ...
    % Correct for the demagnetizing field
    m_cor(x) = m(x) / (1+m(x)*N_d);
    ...
    % m_cor(x) = (m(x)^(-1) - N_d)^(-1);
    fprintf('\n susceptibility: %3.0f pm %3.0f ppb \n', m(x).*1e3, SE_m(x).*1e3);
    
    if x == 4 || x == 5
        chi_ppm_cor(x) = m_cor(x) - m_cor(1);
    end
    
    b(x) = mdl_SI.Coefficients{1,1}; % A/m
    SE_b(x) = mdl_SI.Coefficients{1,2};
    fprintf('\n remanence: %3.0f pm %3.0f %3.0f 10^-3 A/m \n', b(x).*1e3, SE_b(x).*1e3);
    ...
    fprintf('\n coercivity: %3.0f pm %3.0f %3.0f 10^3 A/m \n', x_intercept(x).*1e3, SE_x(x).*1e3);
    ...
    if x == 4 || x == 5
        ...
        if x == 4
            ...
            accuracy(x) = (m_cor(x) - 0.694);
            accuracy_pc(x) = (m_cor(x) - 0.694) / 0.694;
        else
            % accuracy(x)= (m_cor(x) - 0.782);
            % accuracy_pc(x)= (m_cor(x) - 0.782) / 0.782;
            accuracy(x) = (m_cor(x) - 0.687);
            accuracy_pc(x) = (m_cor(x) - 0.687) / 0.687;
        end
    elseif x == 1
    ...
    yticklabels(compose("%3.0f", yticks));
    ...
    a0_term(1) = mdl.Coefficients{1,1};
    a1_term(1) = mdl.Coefficients{2,1};
    a1_term_error(1) = mdl.Coefficients{2,2};
    fprintf('\n a1_term: (%3.0f  pm %3.0f) ppb K.m/A \n', a1_term(1)*1e3, a1_term_error(1)*1e3);
    linear_curve_HvsT = a1_term(1) .* fieldScale .* field_SI_segment + a0_term(1);
    ...
    for idx = 2:divisor
        ...
        field_SI_segment = field_SI(fieldRange);
        mdl = fitlm(fieldScale .* field_SI_segment, T_sample_segment);
        a0_term(idx) = mdl.Coefficients{1,1};
        a1_term(idx) = mdl.Coefficients{2,1};
        a1_term_error(idx) = mdl.Coefficients{2,2};
        fprintf('\n a1_term: (%3.0f  pm %3.0f) ppb K.m/A \n', a1_term(idx)*1e3, a1_term_error(idx)*1e3);
        
        linear_curve_HvsT(:,idx) = a1_term(idx) .* fieldScale .* field_SI_segment + a0_term(idx);
    ...
    yticklabels(compose("%3.0f", round(yticks)));
